...
/*    private void openAddTypeDialog() {
        ...
        H4 dialogHeader = new H4("Add Expense Type");
    ...
    private void openEditTypeDialog(ExpenseTypeDTO expenseTypeDTO) {
        ...
        H4 dialogHeader = new H4("Edit Expense Type");
        dialogHeader.getStyle()
                .set("margin", "0")
                .set("background", "linear-gradient(135deg, #FF7F11, #0A9396)")
                .set("-webkit-background-clip", "text")
...
// Reusable helper to create a consistent header layout for all dialogs
...
public class ExpenseView extends VerticalLayout {
    ...
    public ExpenseView(ExpenseTransactionService expenseTransactionService,
        ...
        H2 header = new H2("Expenses");
        header.getStyle().set("margin", "var(--lumo-space-s)");
        add(header);
    ...
    private void configureTabs() {
        tabs.setWidthFull();
        tabs.getStyle().set("margin", "var(--lumo-space-s)");
    }
    ...
    private void openAddTypeDialog() {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Add Expense Type");
    ...
    private void openEditTypeDialog(ExpenseTypeDTO expenseTypeDTO) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Edit Expense Type");
    ...
    private void openDeleteTypeDialog(ExpenseTypeDTO expenseTypeDTO) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Delete Expense Type");

        Span message = new Span("Are you sure you want to delete the expense type '" + expenseTypeDTO.name() + "'?");
    ...
    private VerticalLayout createTransactionsSection() {
        ...
        HorizontalLayout filterLayout = new HorizontalLayout(recordsFromDatePicker, recordsToDatePicker, filterRecordsButton);
        filterLayout.setAlignItems(Alignment.BASELINE);
        filterLayout.setPadding(false);
        filterLayout.setMargin(false);
        ...
        HorizontalLayout paginationLayout = new HorizontalLayout(pageSizeCombo, prevButton, nextButton, pageLabel);
        paginationLayout.setAlignItems(Alignment.BASELINE);
        paginationLayout.setPadding(false);
        paginationLayout.setMargin(false);
    ...
    private void openBatchEntryDialog() {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Batch Expense Entry");
        dialog.setWidth("800px");
        ...
        ComboBox<Branch> branchComboBox;
        TextField branchTextField = null;
        if (isCompanyAdmin) {
            branchComboBox = new ComboBox<>("Branch");
            branchComboBox.setItems(branchService.getActiveBranchesByCompany(companyId));
        ...
        VerticalLayout formLayout = new VerticalLayout(selectionLayout, entriesForm);
    ...
    private void openEditTransactionDialog(ExpenseTransactionDTO transactionDTO) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Edit Expense Transaction");
        dialog.setWidth("600px");
    ...
    private void openDeleteTransactionDialog(ExpenseTransactionDTO transactionDTO) {
        Dialog dialog = new Dialog();
        dialog.setHeaderTitle("Delete Expense Transaction");

        Span message = new Span("Are you sure you want to delete this transaction for " + transactionDTO.expenseTypeName() + " on " + transactionDTO.transactionDate() + "?");
    ...
    private void configureTypesGrid() {
        typesGrid.addColumn(ExpenseTypeDTO::name).setHeader("Name");
        typesGrid.addColumn(ExpenseTypeDTO::createdAt).setHeader("Created At");
        typesGrid.addColumn(ExpenseTypeDTO::updatedAt).setHeader("Updated At");
        typesGrid.addComponentColumn(dto -> {
            HorizontalLayout actions = new HorizontalLayout();
            Button editButton = new Button("Edit", e -> openEditTypeDialog(dto));
            editButton.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL);
            Button deleteButton = new Button("Delete", e -> openDeleteTypeDialog(dto));
    ...
    private void configureTransactionsGrid() {
        ...
        transactionsGrid.addComponentColumn(dto -> {
            HorizontalLayout actions = new HorizontalLayout();
            Button editButton = new Button("Edit", e -> openEditTransactionDialog(dto));
            editButton.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL);
            Button deleteButton = new Button("Delete", e -> openDeleteTransactionDialog(dto));
            deleteButton.addThemeVariants(ButtonVariant.LUMO_TERTIARY, ButtonVariant.LUMO_SMALL, ButtonVariant.LUMO_ERROR);
            actions.add(editButton, deleteButton);
            return actions;
        }).setHeader("Actions").setWidth("200px");
