...
@Route(value = "company-admin/roles", layout = MainLayout.class)
@PageTitle("Roles & Permissions | BranchOne")
@RolesAllowed("ROLE_COMPANY_ADMIN")
public class CompanyAdminRolesAndPermissionView extends VerticalLayout {
    ...
    @Autowired
    public CompanyAdminRolesAndPermissionView(RoleService roleService, PermissionService permissionService,
    ...
    private VerticalLayout createPermissionSection() {
        ...
        permissionGrid.addColumn(new ComponentRenderer<>(permission -> {
            ...
            tag.getStyle().set("background-color", switch (permission.getCategory()) {
                ...
                case "BRANCH_MANAGEMENT" -> "#E8F5E9";
                case "PRODUCT_MANAGEMENT" -> "#F3E5F5";
                case "STOCK_MANAGEMENT" -> "#E8F5E9";
                default -> "#ECEFF1";
            });
    ...
    private void loadRoles() {
        ...
        roleGrid.setItems(filteredRoles);
    }
    ...
    private void onRoleSelected(Role role) {
        ...
        // Filter permissions for company admin (exclude system admin permissions)
        List<String> allowedCategories = List.of("USER_MANAGEMENT", "COMPANY_MANAGEMENT",
                "BRANCH_MANAGEMENT", "PRODUCT_MANAGEMENT", "STOCK_MANAGEMENT");
    ...
    private void openAddRoleDialog() {
        ...
        systemName.setPrefixComponent(VaadinIcon.USER.create());
        systemName.setHelperText("Will be prefixed with 'ROLE_' automatically");
        ...
        Button saveButton = new Button("Save", e -> {
            ...
            String roleName = systemName.getValue().toUpperCase();
            ...
            if (roleService.findByName(roleName).isPresent()) {
                Notification.show("Role with this system name already exists!", 3000, Notification.Position.MIDDLE);
                return;
            }
    ...
    private void openEditRoleDialog(Role role) {
        if (isSystemRole(role)) {
            Notification.show("System roles cannot be edited.", 3000, Notification.Position.MIDDLE);
            return;
        }
    ...
    private void deleteRole(Role role) {
        if (isSystemRole(role)) {
            Notification.show("System roles cannot be deleted.", 3000, Notification.Position.MIDDLE);
            return;
        }
        ...
        confirmDialog.setText("Are you sure you want to delete the role '" + role.getDisplayName() + "'? " +
        ...
        confirmDialog.addConfirmListener(e -> {
            try {
                ...
                // You might want to add a method in UserService to check this
                roleService.getAllRoles().remove(role); // This is simplified - you'll need proper deletion logic
                Notification.show("Role deleted successfully!");
                loadRoles();
            } catch (Exception ex) {
                Notification.show("Error deleting role: " + ex.getMessage(), 5000, Notification.Position.MIDDLE);
            }
        });
    ...
    private boolean isSystemRole(Role role) {
        return role.getName().equals("ROLE_SUPER_ADMIN") ||
                role.getName().equals("ROLE_COMPANY_ADMIN") ||
                role.getName().equals("ROLE_BRANCH_MANAGER") ||
                role.getName().equals("ROLE_SHIFT_SUPERVISOR") ||
                role.getName().equals("ROLE_SHIFT_ATTENDANT");
    }
}
